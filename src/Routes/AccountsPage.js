...
import { MdGroups2, MdOutlineMessage } from 'react-icons/md'
import { BiSearch } from "react-icons/bi"
import { BsFilter } from "react-icons/bs"
import { CiMenuKebab } from 'react-icons/ci'
import { TbCircleDashed } from 'react-icons/tb'
import { getUsersData } from "../API/Accounts"
...
const AccountsPage = () => {
  ...
  const [chartsPageDisplay, setChartsPageDisplay] = useState("")
  const [visibleAuth, setVisibleAuth] = useState("")

  // GET call for getting all the user accounts
  const getAllUserAccounts = async () => {
    try {
      const resOfUserAccounts = await getUsersData()
      const getAuthRes = await getAuthData()
      Promise.all([resOfUserAccounts, getAuthRes]).then((values) => {
        setUserAccounts(values[0])
        setAuthData(values[1])
      })
    }
    catch (e) {
      console.log(e)
  ...
  useEffect(() => { getAllUserAccounts() }, [])

  //formating timestamp date to local time strring
  ...
  const onSearchContacts = (e) => {
    let searchKey = (e.target.value).toString()
    const accountsFound = userAccounts.filter((contactName) => contactName.name.toLocaleLowerCase().includes(searchKey.toLocaleLowerCase()))
    if (searchKey === "") {
      getAllUserAccounts()
    }
    else {
      setUserAccounts(accountsFound)
    }
  }

  const onEnableAccountsPage = (usersInfo) => {
    setChartsPageDisplay(usersInfo)
  }

  const onDisplayAccounts = (usersInfo) => {
    return (
      <MessagesPage usersInfo={usersInfo} />
    )
  }

  const onClickOnAuthProfile = (auth) => {
    setVisibleAuth(auth)
  }

  const displayAuthPage = (auth) => {
    return (
      <AuthPage auth={auth} />
    )
  }
        ...
        <div style={{ backgroundColor: COLORS.CINDER, display: "flex", flexDirection: "row", justifyContent: "space-between", padding: "5px" }}>
          <div>
            {authData?.map((auth) => {
              return (
                <>
                  <Link to={"/?profile"} key={authData?._id}>
                    {
                      auth?.image !== "" && <img src={auth?.image} height={40} width={40} style={{ borderRadius: "40px", cursor: "pointer" }} alt={""} key={auth?._id} onClick={() => onClickOnAuthProfile(auth)} />
                    }
                  </Link>
                </>
              )
            ...
            <Link to={"/?profile"} key={authData?._id}>
              {
                ...
                authRes?.image === "" && <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR0ZL7it9GAAc4a1Fb40d6fxu-paaRZ-zG2yQ&usqp=CAU" height={40} width={40} style={{ borderRadius: "40px", cursor: "pointer" }} alt={""} key={authData?._id} onClick={() => onClickOnAuthProfile(authRes)} />
              }
            </Link>
          </div>
          <div style={{ marginTop: "10px" }}>
            <MdGroups2 size={25} color={COLORS.BRIGHT_GREY} style={{ cursor: "pointer" }} />
            <TbCircleDashed size={25} color={COLORS.BRIGHT_GREY} style={{ marginLeft: "20px", cursor: "pointer" }} />
            <MdOutlineMessage size={25} color={COLORS.BRIGHT_GREY} style={{ marginLeft: "20px", cursor: "pointer" }} />
            <CiMenuKebab size={25} color={COLORS.BRIGHT_GREY} style={{ marginLeft: "20px", cursor: "pointer" }} />
          </div>
        </div>
        <div className='homeContainer' style={{ marginTop: "10px", padding: "5px", height: "35px" }}>
          <div style={{
            backgroundColor: COLORS.CINDER, borderRadius: "8px", marginLeft: "10px", width: "35vw", padding: "5px"
          }}>
            <BiSearch size={20} color={COLORS.BRIGHT_GREY} style={{ cursor: "pointer" }} />
            <input type={"text"} placeholder='search or start new chart' style={{
              backgroundColor: COLORS.CINDER, border: "hidden", marginLeft: "20px", color: "white", width: "30vw", marginBottom: "5px"
            }} onChange={
              onSearchContacts
            } />
          </div>
          <div>
            <BsFilter size={25} color={COLORS.BRIGHT_GREY} style={{ cursor: "pointer" }} />
          </div>
        </div>
  ...
  return (
    <div className='homeContainer'>
      <div className="accountPortion">
        {visibleAuth === "" ? displayAccountsPage() : displayAuthPage(visibleAuth)}
      </div>
      <div className="chartsPortion">
        {chartsPageDisplay !== "" ? onDisplayAccounts(chartsPageDisplay) : onDisplayInitialChartsScreen()}
      </div>
    </div>
